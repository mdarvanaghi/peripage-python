...
class PrinterTypeSpecs:
    ...
    * `aspect_correction` - compensates for printers whose physical paper
      feed distance per printed row does not match their horizontal dot
      pitch, which otherwise makes square images print visibly non-square.
      A value < 1.0 shrinks the number of rows sent (compresses vertically
      relative to the naive width-preserving-aspect-ratio scale); > 1.0
      stretches it. Default `1.0` means "no correction" - the printer's
      feed distance per row already matches its dot pitch, or this simply
      hasn't been measured/calibrated yet.
    * `has_onboard_font` - whether the printer's firmware can rasterize
      raw ASCII bytes itself (true for the A6-series - confirmed by the
      original protocol reverse-engineering this module extracted). BLE
      "cat printer"-style devices like the P21 have no such firmware
      feature; sending them raw ASCII just feeds blank paper. Controls
      which strategy `Printer.printText()` uses - see that method's
      docstring for the reasoning and sources.
    """
...
class PrinterType(enum.Enum):
    ...
    # 384 dots / 48 bytes / 32 chars, matching the A6's row geometry -
    # correct for the P21's 57mm x 40mm (58mm-roll-class) paper at 203DPI.
    # aspect_correction compensates for a small mismatch between the
    # printer's paper-feed distance per row and its horizontal dot pitch;
    # calibrate against the actual transport configuration in use (see
    # scripts/print_bar.py), since transport-level
    # write throughput can otherwise masquerade as a geometry issue.
    P21 = PrinterTypeSpecs(
        row_bytes=48,
        row_width=384,
        row_characters=32,
        aspect_correction=45 / 48,
        # This printer class has no onboard font/text mode and only ever
        # prints bitmaps - see Printer.printText() for how this module
        # works around that.
        has_onboard_font=False,
    )
...
class Printer:
    ...
    def __init__(self, transport: Transport, printer_type: PrinterType):
        """
        Create instance of peripage connector. `transport` carries bytes to
        and from the physical printer (classic Bluetooth, BLE, or a fake
        transport for testing) and `printer_type` supplies the
        printer-specific printing parameters.
        ...
        * `transport` - a `peripage.transport.Transport` instance, e.g.
          `SocketTransport` (classic Bluetooth: A6/A6+/A40/A40+) or
          `BleakTransport` (BLE: P21).
    ...
    def connect(self) -> None:
        ...
        printer, the connection may become unoperateable.
    ...
    def setTimeout(self, timeout) -> None:
        """
        Set the transport connection recv / send timeout.
        """
    ...
    def askPrinter(self, byteseq: bytes, recv_size: int=1024) -> bytes:
        ...
        * `byteseq` - `bytes` data
        ...
        self.transport.send(byteseq)
        return self.transport.recv(recv_size)
    ...
    def printImage(self, img: PIL.Image.Image, delay=0.01, resample=PIL.Image.Resampling.NEAREST) -> None:
        ...
        * `resample` - resampling mode of the image, used to automatically
        ...
        target_height = self.getRowWidth() / img.size[0] * img.size[1] * self.printer_type.spec.aspect_correction
        img = img.resize((self.getRowWidth(), max(1, round(target_height))), resample)
        img = img.convert('1')
    ...
    def printText(
        ...
        """
        Print arbitrary text, choosing the right strategy for the
        connected printer automatically:

        * On printers with an onboard font (`PrinterTypeSpecs.
          has_onboard_font=True` - the A6-series, confirmed by the
          original reverse-engineering this module extracts its protocol
          from) - sends raw ASCII via `printASCII()`/`flushASCII()`,
          exactly like the original implementation.
        * On printers with no onboard font (`has_onboard_font=False` -
          the P21 and other BLE "cat printer"-style devices) - renders
          the text to an image with `peripage.text_render.render_text()`
          and prints it via `printImage()`. This is not a limitation of
          this module: independent reverse-engineering of the same
          printer class confirms these printers have no text mode at all
          and only ever print bitmaps (see `PrinterType.P21`'s source
          comment). Unlike raw-ASCII printing, this path also supports
          arbitrary Unicode (subject to font coverage), custom font
          sizes, and alignment.

        Arguments:
        * `text` - the text to print. May contain newlines.
        * `font_size` - font size in points, only used on the
          image-rendering path.
        * `font_path` - path to a `.ttf`/`.otf` font; defaults to the
          bundled DejaVu Sans Mono. Only used on the image-rendering path.
        * `align` - `'left'`, `'center'`, or `'right'`. Only used on the
          image-rendering path.
        * `line_spacing` - extra pixels between lines. Only used on the
          image-rendering path.
        * `delay` - delay between printing each row of the rendered
          image. Only used on the image-rendering path.
        * `resample` - resampling mode used to fit the rendered image to
          `Printer.getRowWidth()`. Only used on the image-rendering path.
        """

        if self.printer_type.spec.has_onboard_font:
            self.printASCII(text)
            self.flushASCII()
            return

        from peripage.text_render import render_text

        img = render_text(
            text,
            width_px=self.getRowWidth(),
            font_size=font_size,
            font_path=font_path,
            align=align,
            line_spacing=line_spacing,
        )
        self.printImage(img, delay=delay, resample=resample)
